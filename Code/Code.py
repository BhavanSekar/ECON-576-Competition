...
import random
random.seed(10)
#Drop the id column as that has is not meant to be a feature for prediction.
df_d=df_d.drop("Id", axis='columns')
#The last column is the output variables. Rest all are features
X = df_d.iloc[:, :-1]
y = df_d.iloc[:, -1]
...
#Split into training and test sets. test set would be 30% the size of the dataset.
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=0)
#Fir simple regression model.
...
#We get whopping r2 of 1! There is no neccesity for model tuning or using any other methods as 
#simple linear regression is giving the maximum r2 possible.
...
df_d_val
#Equalize the feature vectors for test and train set: If a category does not exist in any set a dummy column might be
